...
static void power_iteration_eigenvalues(const float* matrix, size_t size,
    ...
    *count = size < max_count ? size : max_count;

    float* vec = (float*)safe_malloc(size * sizeof(float));
    if (!vec) { *count = 0; return; }

    for (size_t eig = 0; eig < *count; eig++) {
        for (size_t i = 0; i < size; i++) vec[i] = (float)((i + eig * 7) % 5) / 5.0f + 0.1f;
        ...
        for (int iter = 0; iter < 50; iter++) {
            float* av = (float*)safe_malloc(size * sizeof(float));
            if (!av) goto cleanup;
            memset(av, 0, size * sizeof(float));

            for (size_t i = 0; i < size; i++) {
                for (size_t j = 0; j < size; j++) {
                    av[i] += matrix[i * size + j] * vec[j];
                }
            }

            eigenvalue = 0.0f;
            for (size_t i = 0; i < size; i++) eigenvalue += vec[i] * av[i];

            norm = 0.0f;
            for (size_t i = 0; i < size; i++) norm += av[i] * av[i];
            if (norm > 1e-15f) {
                float inv_norm = 1.0f / sqrtf(norm);
                for (size_t i = 0; i < size; i++) av[i] *= inv_norm;
            }

            memcpy(vec, av, size * sizeof(float));
            safe_free((void**)&av);

            if (fabsf(eigenvalue - prev_eigenvalue) < 1e-5f) break;
            prev_eigenvalue = eigenvalue;
        }

        real_parts[eig] = eigenvalue;
        imag_parts[eig] = 0.0f;

        float* deflated = vec;
        for (size_t i = 0; i < size; i++) {
            for (size_t j = 0; j < size; j++) {
                deflated[i] -= eigenvalue * vec[i] * vec[j];
            }
        }
    }
