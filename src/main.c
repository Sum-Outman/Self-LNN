...
static void* volatile g_global_lnn = NULL;                      /* 修复#5: 改为static，外部模块须通过selflnn_get_shared_lnn()安全访问；volatile防止编译器在单线程路径中缓存指针值 */
...
static float lnn_weights_fitness_function(const float* chromosome, size_t chrom_size, void* user_data) {
    ...
    /* 修复#7: 获取锁后保存原始权重，消除TOCTOU窗口（原来在锁外保存，存在竞态风险） */
    lnn_lock(lnn);
