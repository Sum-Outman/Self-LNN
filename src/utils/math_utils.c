 ...
 * 从auth.c提取，支持流式接口（init/update/final）和便捷一次性接口
 * ================================================================ */
...
static void sha256_transform_internal(selflnn_sha256_ctx_t* ctx, const uint8_t data[]) {
...
void selflnn_sha256_update(selflnn_sha256_ctx_t* ctx, const uint8_t* data, size_t len) {
    ...
    for (size_t i=0; i<len; i++) {
        ...
        if (ctx->datalen == 64) {
            sha256_transform_internal(ctx, ctx->data);
            ctx->bitlen += 512;
            ctx->datalen = 0;
        }
    }
...
void selflnn_sha256_hash(const uint8_t* input, size_t input_len, uint8_t output[SELFLNN_SHA256_HASH_LEN]) {
    ...
    selflnn_sha256_update(&ctx, input, input_len);
