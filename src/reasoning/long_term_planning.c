 ...
 * M-023修复: 职责边界标注。
 * 
 * 【long_term_planning.c 职责边界】
 *   本文件专注于【目标分解与时间线规划】，核心是：
 *   - 分层目标分解（HGD）：将长期目标拆解为可执行的子目标序列
 *   - 时序逻辑规划（TLP）：处理子目标间的时序约束（Allen区间代数）
 *   - 资源受限项目调度（RCPS）：在资源约束下优化执行时间表
 *   - 自适应重规划：根据执行反馈动态调整时间线和资源分配
 *   输出为【带时序约束的子目标执行计划】，不负责子任务内部结构。
 * 
 * 【与 hierarchical_planning.c 的关系】
 *   - hierarchical_planning.c 负责【任务分解 + 层次结构】
 *     （将单个任务按抽象层级拆解为原子子任务）
 *   - 本文件负责【目标分解 + 时序规划 + 资源调度】
 *     （决定子目标的执行顺序、时间窗口和资源分配）
 *   - 两者通过 planning_enhanced.c 中的统一调度接口路由：
 *     planning_enhanced 先调用本模块做目标分解与时间排程，
