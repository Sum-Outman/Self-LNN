...
static const KeywordRule keyword_rules[] = {
    {"手机", PRODUCT_TYPE_HARDWARE, "移动通信", 1.5},
    {"电脑", PRODUCT_TYPE_HARDWARE, "计算处理", 2.0},
    {"软件", PRODUCT_TYPE_SOFTWARE, "程序代码", 1.0},
    {"应用", PRODUCT_TYPE_SOFTWARE, "用户界面", 0.8},
    {"系统", PRODUCT_TYPE_SYSTEM, "集成平台", 3.0},
    {"智能", PRODUCT_TYPE_HARDWARE, "人工智能", 2.5},
    {"联网", PRODUCT_TYPE_SYSTEM, "网络连接", 1.2},
    {"安全", PRODUCT_TYPE_SOFTWARE, "安全防护", 1.3},
    {"快速", PRODUCT_TYPE_HARDWARE, "高性能", 1.4},
    {"便宜", PRODUCT_TYPE_HARDWARE, "低成本", 0.7},
    {"可靠", PRODUCT_TYPE_SYSTEM, "高可靠性", 1.6},
    {"易用", PRODUCT_TYPE_SOFTWARE, "用户体验", 1.1}
};
...
/* ============================================================================
 * 5.3 修复: 产品设计系统 — 需求解析 → 规格生成 → 设计评估
 * ============================================================================ */

int product_design_parse_requirement(ProductDesignEngine* engine,
                                      const char* requirement_text,
                                      ProductRequirement* req_out) {
    if (!engine || !requirement_text || !req_out) return -1;
    memset(req_out, 0, sizeof(ProductRequirement));

    size_t kw_count = 0;
    char** keywords = extract_keywords(requirement_text, &kw_count);
    if (keywords && kw_count > 0) {
        req_out->keywords = keywords;
        req_out->keyword_count = kw_count;
        req_out->preferred_type = detect_product_type((const char**)keywords, kw_count);
        double complexity = estimate_complexity((const char**)keywords, kw_count);
        req_out->max_cost = estimate_cost(req_out->preferred_type, complexity) * 1.5;
        req_out->max_time = estimate_time(req_out->preferred_type, complexity) * 1.2;
    }
    return 0;
}

int product_design_generate_spec(ProductDesignEngine* engine,
                                  const ProductRequirement* req,
                                  ProductSpec* spec_out) {
    if (!engine || !req || !spec_out) return -1;
    memset(spec_out, 0, sizeof(ProductSpec));
    spec_out->type = req->preferred_type;
    spec_out->estimated_cost = req->max_cost * 0.7;
    spec_out->development_time = req->max_time * 0.8;

    double complexity = 1.0;
    if (req->keywords && req->keyword_count > 0)
        complexity = estimate_complexity((const char**)req->keywords, req->keyword_count);
    spec_out->complexity_score = complexity;
    spec_out->feasibility_score = 1.0 / (1.0 + complexity * 0.3);
    return 0;
}

int product_design_evaluate(ProductDesignEngine* engine,
                             const ProductRequirement* req,
                             const ProductSpec* spec,
                             float* overall_score) {
    if (!engine || !req || !spec || !overall_score) return -1;
    float w_f = 0.30f, w_c = 0.25f, w_q = 0.25f, w_i = 0.20f;
    float cost_norm = spec->estimated_cost > 0 ? (float)(1.0 / (1.0 + spec->estimated_cost / 100000.0)) : 1.0f;
    *overall_score = (float)spec->feasibility_score * w_f +
                     cost_norm * w_c +
                     (float)spec->complexity_score / 10.0f * w_q +
                      (req->preferred_type != 0 ? 0.7f : 0.5f) * w_i;
    return 0;
}