...
| **后端服务** | HTTP REST API（359 handler槽位，359个API枚举 + 8个向后兼容别名）、WebSocket 实时通信、API 密钥认证、安全监控 |
...
```
> **power_mode可选值：** `"low"`/`"power_saving"`（节能）、`"balanced"`（均衡，默认）、`"high"`/`"performance"`（性能）
...
| 7 | `ODE_SOLVER_RHS` | 直接右端项评估（前向欧拉，备选） / Direct RHS evaluation (forward Euler, fallback) |
...
| GET | /api/cognition/status | 获取AGI认知状态（别名） |
...
| POST | /api/dialogue/history/save | 保存对话历史（持久化） |
| POST | /api/dialogue/clear | 清除对话历史 |
| POST | /api/dialogue/multimodal | 多模态对话 |
| POST | /api/dialogue/send | 发送对话消息（别名） |
...
| POST | /api/kg/add | 添加知识图谱节点/边 | `{"type":"node","label":"实体名",...}` |
| POST | /api/kg/delete | 删除知识图谱节点/边 | `{"type":"node","label":"实体名"}` |
...
| POST | /api/training/logs/clear | 清除训练日志（也可使用 `/api/training/log/clear` 别名） |
| POST | /api/training/resume | 恢复训练任务 |
...
| 方法 | 路径 | 描述 |
|--------------|------------|-------------------|
| GET | /api/audio/devices | 枚举音频采集设备（麦克风） |
...
| 方法 | 路径 | 描述 |
|--------------|------------|-------------------|
| POST | /api/camera/switch | 摄像头切换 |
| POST | /api/video/quality | 视频质量 |
| GET | /api/camera/devices | 摄像头设备列表 |
| POST | /api/camera/capture/start | 开始采集 |
| POST | /api/camera/capture/stop | 停止采集 |
...
| POST | /api/device/control | 设备开关控制 |
...
| 方法 | 路径 | 描述 |
|--------------|------------|-------------------|
| POST | /api/product/spec | 产品规格生成 |
| GET | /api/product/status | 产品设计引擎状态 |
| POST | /api/product/design | 产品设计请求 |
...
| GET | /api/safety/bounds | 获取安全边界配置 |
| POST | /api/safety/bounds | 更新安全边界配置 |
...
| **原子交换 / Atomic swap** | 新旧 LNN 指针通过 `InterlockedExchangePointer`（Windows）/ `__atomic_exchange_n(ACQ_REL)`（GCC/Clang）实现真正的原子替换，推理不中断 / Old→new LNN pointer atomic swap via true hardware atomics, no inference interruption |
...
> **文档更新**: 2026-07-04 | **修复状态**: 全系统审计修复完成（79项缺陷100%修复）
> **开源仓库**: https://github.com/Sum-Outman/Self-LNN
> **开发者邮箱**: silencecrowtom@qq.com
