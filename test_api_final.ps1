...
function Test-GET($path) {
    try {
        $r = Invoke-WebRequest -Uri "$BASE$path" -Method GET -UseBasicParsing -TimeoutSec $TIMEOUT -ErrorAction Stop
        if ($r.StatusCode -eq 200) { $script:pass++ } 
        else { $script:fail++; $script:results += "$path -> $($r.StatusCode)" }
    } catch {
        $sc = if ($_.Exception.Response) { $_.Exception.Response.StatusCode.value__ } else { "TIMEOUT" }
...
function Test-POST($path, $body) {
    try {
        $r = Invoke-WebRequest -Uri "$BASE$path" -Method POST -Body $body -ContentType "application/json" -UseBasicParsing -TimeoutSec $TIMEOUT -ErrorAction Stop
        if ($r.StatusCode -eq 200) { $script:pass++ }
        else { $script:fail++; $script:results += "$path -> $($r.StatusCode)" }
    } catch {
        $sc = if ($_.Exception.Response) { $_.Exception.Response.StatusCode.value__ } else { "TIMEOUT" }
...
Write-Host "总响应率: $([Math]::Round($pass/($pass+$fail)*100, 1))%"
