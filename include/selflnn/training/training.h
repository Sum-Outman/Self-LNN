 ...
 *
 * P3-004 功能边界说明:
 *   ✅ 本文件: 核心训练算法（梯度下降、反向传播、批量/小批量/在线/自适应模式）
 *   ✅ 本文件: 基础正则化（L1/L2/Dropout/早停）、优化器（SGD/Adam/RMSprop）
 *   ✅ 本文件: 模型版本管理、训练状态保存/恢复
 *   ❌ 非本文件: EMA权重管理、Warmup+余弦退火调度器、知识蒸馏、混合精度训练、
 *                CfC ODE特有训练、拉普拉斯增强训练
 *                → 请使用 training_enhanced.h（高级增强功能，与本基础版互补，非替代）
 *
